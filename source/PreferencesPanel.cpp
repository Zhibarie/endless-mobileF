...
#ifdef __ANDROID__
	controlTypeDropdown->SetText(SHOW_GESTURES);
#endif
...
bool PreferencesPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
	if(static_cast<unsigned>(editing) < zones.size())
	{
		...
		controlTypeDropdown->SetText(SHOW_KEYS);
		EndEditing();
		return true;
	}
	...
	else if (page == 'p' && key == 'i')
	{
		...
		if (success)
		{
			GetUI().Push(DialogPanel::CallFunctionIfOk(&GetUI(), &UI::Quit, "Plugin installed. Endless Sky needs to be restarted."));
		}
	...
	else if (page == 'p' && key == 'r')
	{
		...
		if(plugin)
		{
			...
			{
				GetUI().Push(DialogPanel::CallFunctionIfOk(&GetUI(), &UI::Quit, "Plugin removed. Endless Sky needs to be restarted."));
			}
			else
			{
				GetUI().Push(DialogPanel::Info("Failed to remove plugin."));
			}
		}
	}
#else
...
bool PreferencesPanel::FingerDown(int x, int y, int fid)
{
	...
	{
		if(controlTypeDropdown->Text() == SHOW_GESTURES)
			Command::SetGesture(zones[editing].Value(), Gesture::NONE);
		else if(controlTypeDropdown->Text() == SHOW_GAMEPAD)
		{
